                ...
                <tbody>
                    ...
                    <?php foreach ($data['table'] as $rows) :?>
                        <tr>
                            ...
                            <td><?= $rows['fakultas'];?></td>
                            <td><?= $rows['kejuruan'];?></td>
                            <td><a href="<?= URLROOT; ?>/mahasiswa/hapus/<?= $rows['id'];?>"
                             onclick="return confirm('yakin mau anda hapus ?')">Hapus Data |
                            <a href="<?= URLROOT; ?>/mahasiswa/pilih/<?= $rows['id'];?>">Pilih Data </td>
                        </tr>
                    <?php endforeach;?>
                </tbody>
            </table>

            <!-- start pagination fungsi default-->
            <ul class="pagination">
                ...
                <?php for($i=1; $i <= $data['dataPage']; $i++) :?>
                    ...
                    <?php if($i == $data['halAktif']) : ?>
                        <li class="active page-item">
                            <a href="<?= URLROOT; ?>/mahasiswa/page/<?= $i;?>" class="page-link"><?= $i;?></a>
                        </li>
                    <?php else : ?>
                        <?php if($i > $data['halAktif']) : ?>
                            <li class="page-item">
                ...
                <?php if($data['next'] !== $data['halAktif'] && $data['halAktif'] < $data['dataPage']): ?>
                    <li class="page-item">
                        <a href="page/<?= $data['next'];?>" class="page-link">&raquo;</a>
                    </li>
                <?php endif;?>
                ...
                <?php if($data['left'] !== $data['halAktif'] && $data['halAktif'] < $data['dataPage']) : ?>
                    <li class="page-item">
                        <a href="page/<?= $data['left'];?>" class="page-link">&rsaquo;</a>
                    </li>
                <?php endif;?>

            </ul>
            <!-- end pagination fungsi default-->
        </div>
    </div>
</div>
